...
BondBPMBeam::BondBPMBeam(LAMMPS *_lmp) :
    BondBPM(_lmp), E(nullptr), G(nullptr), ecrit(nullptr), thetacrit(nullptr), psicrit(nullptr),
    gnorm(nullptr), gslide(nullptr), groll(nullptr), gtwist(nullptr)
{
  partial_flag = 1;
  smooth_flag = 1;
  normalize_flag = 0;
  ...
  nhistory = 4;
  id_fix_bond_history = utils::strdup("HISTORY_BPM_BEAM");
...
BondBPMBeam::~BondBPMBeam()
{
  ...
  if (allocated) {
    ...
    memory->destroy(thetacrit);
    memory->destroy(psicrit);
...
double BondBPMBeam::elastic_forces(int i1, int i2, int type, double r_mag, double r0_mag,
                                         double r_mag_inv, double * /*rhat*/, double *r, double *r0,
                                         double *force1on2, double *torque1on2, double *torque2on1, double &ebond)
{
  double breaking;
  double temp, r0_dot_rb, c, gamma;
  ...
  double E_type = E[type];
  double G_type = G[type];

  double Rsq1 = radius[i1]*radius[i1];
  double Rsq2 = radius[i2]*radius[i2];

  if (fabs(Rsq1-Rsq2) > EPSILON) error->warning(FLERR, "BPM bond/beam assumes uniform atom radii");

  double A = 0.5 * MY_PI * (Rsq1 + Rsq2);
  double I = 0.125 * MY_PI * (Rsq1*Rsq1 + Rsq2*Rsq2);
  double J = 2.0 * I;

  // Based on Euler-Bernoulli beam theory assuming a circular cross-section with radius equal to atom radius
  double Kr_type = E_type * A * r0_mag_inv;
  double Ks_type = 12.0 * E_type * I * r0_mag_inv*r0_mag_inv*r0_mag_inv;
  double Kb_type = E_type * I * r0_mag_inv;
  double Kt_type = G_type * J * r0_mag_inv;
  ...
  // Energy due to shear force and torque
  double rb_mag = sqrt(MathExtra::dot3(rb, rb));
  ebond += 0.5 * Ks_type * r_mag * gamma * r_mag * gamma;
  ebond -= 0.5 * r_mag * Ks_type * r_mag * gamma * theta;
  ...
  Tbp[0] = -Kb_type * theta * sin_phi;
  Tbp[1] = Kb_type * theta * cos_phi;
  Tbp[2] = 0.0;

  ebond += 2.0 * Kb_type * theta * theta;

  Ttp[0] = 0.0;
  Ttp[1] = 0.0;
  Ttp[2] = Kt_type * psi;

  ebond += 0.5 * Kt_type * psi * psi;
  ...
  breaking = fabs(e) / ecrit[type] + fabs(theta) / thetacrit[type] + fabs(psi) / psicrit[type];
...
void BondBPMBeam::damping_forces(int i1, int i2, int type, double *rhat, double *r,
...
void BondBPMBeam::compute(int eflag, int vflag)
{
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    double breaking = elastic_forces(i1, i2, type, r_mag, r0_mag, r_mag_inv, rhat, r, r0, force1on2, torque1on2, torque2on1, ebond);
    ...
    if (evflag)
      ev_tally_xyz(i1, i2, nlocal, newton_bond, ebond * smooth, -force1on2[0] * smooth, -force1on2[1] * smooth,
...
void BondBPMBeam::allocate()
{
  ...
  memory->create(E, np1, "bond:E");
  memory->create(G, np1, "bond:G");
  memory->create(ecrit, np1, "bond:ecrit");
  memory->create(thetacrit, np1, "bond:thetacrit");
  memory->create(psicrit, np1, "bond:psicrit");
...
void BondBPMBeam::coeff(int narg, char **arg)
{
  ...
  double ecrit_one = utils::numeric(FLERR, arg[3], false, lmp);
  double thetacrit_one = utils::numeric(FLERR, arg[4], false, lmp);
  double psicrit_one = utils::numeric(FLERR, arg[5], false, lmp);
  double gnorm_one = utils::numeric(FLERR, arg[6], false, lmp);
  double gslide_one = utils::numeric(FLERR, arg[7], false, lmp);
  double groll_one = utils::numeric(FLERR, arg[8], false, lmp);
  double gtwist_one = utils::numeric(FLERR, arg[9], false, lmp);
  ...
  for (int i = ilo; i <= ihi; i++) {
    ...
    G[i] = G_one;
    ecrit[i] = ecrit_one;
    thetacrit[i] = thetacrit_one;
    psicrit[i] = psicrit_one;
    gnorm[i] = gnorm_one;
    gslide[i] = gslide_one;
    groll[i] = groll_one;
    gtwist[i] = gtwist_one;
...
void BondBPMBeam::init_style()
{
  ...
  if (!atom->quat_flag || !atom->radius_flag || !atom->omega_flag)
    error->all(FLERR, "Bond bpm/beam requires atom style bpm/sphere");
  if (comm->ghost_velocity == 0)
    error->all(FLERR, "Bond bpm/beam requires ghost atoms store velocity");
...
void BondBPMBeam::write_restart(FILE *fp)
{
  ...
  fwrite(&thetacrit[1], sizeof(double), atom->nbondtypes, fp);
  fwrite(&psicrit[1], sizeof(double), atom->nbondtypes, fp);
...
void BondBPMBeam::read_restart(FILE *fp)
{
  ...
  if (comm->me == 0) {
    utils::sfread(FLERR, &E[1], sizeof(double), atom->nbondtypes, fp, nullptr, error);
    utils::sfread(FLERR, &G[1], sizeof(double), atom->nbondtypes, fp, nullptr, error);
    utils::sfread(FLERR, &ecrit[1], sizeof(double), atom->nbondtypes, fp, nullptr, error);
    utils::sfread(FLERR, &thetacrit[1], sizeof(double), atom->nbondtypes, fp, nullptr, error);
    utils::sfread(FLERR, &psicrit[1], sizeof(double), atom->nbondtypes, fp, nullptr, error);
  ...
  MPI_Bcast(&thetacrit[1], atom->nbondtypes, MPI_DOUBLE, 0, world);
  MPI_Bcast(&psicrit[1], atom->nbondtypes, MPI_DOUBLE, 0, world);
...
void BondBPMBeam::write_restart_settings(FILE *fp)
{
...
void BondBPMBeam::read_restart_settings(FILE *fp)
{
...
double BondBPMBeam::single(int type, double rsq, int i, int j, double &fforce)
{
  // Not yet enabled
  return 0.0;
}
